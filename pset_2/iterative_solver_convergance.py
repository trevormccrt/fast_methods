import matplotlib.pyplot as plt
import numpy as np
from scipy import io as scio

import util, helmholtz_solver

data = scio.loadmat("MRI_DATA.mat")
eps = np.flipud(data["e_r"][1:-1, 1:-1])

grid_spacing = 1/256
grid = np.arange(start=0, stop=1 + grid_spacing, step=grid_spacing)
grid_free = grid[1:-1]
x_grid, y_grid = np.meshgrid(grid_free, grid_free)


center_x = 0.5
center_y = 0.5
sigma = 0.01
v = util.gaussian_2d(x_grid, y_grid, center_x, center_y, sigma)


xlims = [0.35, 0.635]
ylims = [0.186, 0.629]


c = 3e8

fig, axs = plt.subplots(nrows=1, ncols=1)

f = 298.3e6
omega = 2 * np.pi * f
k_sq = omega ** 2 / c ** 2
soln_direct = helmholtz_solver.solve_direct_fd_variable_k(grid_spacing, k_sq * eps, v)
new_state = np.zeros_like(soln_direct)
deltas = []
for _ in range(30):
    new_state = helmholtz_solver.run_iteration_variable_k(grid_spacing, k_sq * eps, k_sq, v, new_state)
    deltas.append(np.max(np.abs(new_state - soln_direct))/np.max(np.abs(soln_direct)))
axs.plot(deltas, label="f = {}".format(f))
axs.set_yscale("log")
plt.show()
print("")
